    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            if (action.bindings[bindingIndex].isPartOfComposite)
            {
                ...
                partName = m_LocalizedBindingPartPrefix.IsEmpty
                ? $"Binding '{bindingName}'. "
                : string.Format(m_LocalizedBindingPartPrefix.GetLocalizedString(), bindingName);
            }

            m_RebindOverlay?.SetActive(true);

            // --- LOCALIZED WAITING PROMPT ---
            if (m_RebindText != null)
            {
                string localizedWaiting;
                if (!string.IsNullOrEmpty(m_RebindOperation.expectedControlType))
                {
                    // Looks for a key matching the control type, otherwise falls back to default prompt
                    localizedWaiting = m_LocalizedWaitingWithControlPrompt.IsEmpty
                        ? $"{partName}Waiting for {m_RebindOperation.expectedControlType} input..."
                        : $"{partName}{string.Format(m_LocalizedWaitingWithControlPrompt.GetLocalizedString(), m_RebindOperation.expectedControlType)}";
                }
                else
                {
                    localizedWaiting = m_LocalizedWaitingPrompt.IsEmpty
                        ? $"{partName}Waiting for input..."
                        : $"{partName}{m_LocalizedWaitingPrompt.GetLocalizedString()}";
                }
                m_RebindText.text = localizedWaiting;
            }
            ...
            // --- LOCALIZED FALLBACK WAITING ---
            if (m_RebindOverlay == null && m_RebindText == null && m_RebindStartEvent == null && m_BindingText != null)
            {
                m_BindingText.text = m_LocalizedFallbackWaitingText.IsEmpty
                    ? "<Waiting...>"
                    : m_LocalizedFallbackWaitingText.GetLocalizedString();
            }
        ...
        private void UpdateRebindInfo(double now)
        {
            ...
            // --- LOCALIZED TIMEOUT DESCRIPTION ---
            var text = string.Empty;
            if (m_RebindOperation.timeout > 0.0f)
            {
                text = m_LocalizedTimeoutDescription.IsEmpty
                    ? $"Cancels in <b>{remainingTimeoutWholeSeconds}</b> seconds if no matching input is provided."
                    : string.Format(m_LocalizedTimeoutDescription.GetLocalizedString(), remainingTimeoutWholeSeconds);
            }
        ...
        [SerializeField] private LocalizedString m_LocalizedWaitingPrompt;
        [SerializeField] private LocalizedString m_LocalizedWaitingWithControlPrompt;
        [SerializeField] private LocalizedString m_LocalizedTimeoutDescription;
        [SerializeField] private LocalizedString m_LocalizedFallbackWaitingText;
        [SerializeField] private LocalizedString m_LocalizedBindingPartPrefix;
